...
class Tracklet(object):
    ...
    Tracklets are created in function parse_xml and can most conveniently used as follows:

    for trackletObj in parse_xml(trackletFile):
      for translation, rotation, state, occlusion, truncation, amt_occlusion, amt_borders, absoluteFrameNumber in trackletObj:
        ... your code here ...
      #end: for all frames
...
def parse_xml(tracklet_file):
    ...
    if tracklet_idx != num_tracklets:
        warn('According to xml information the file has {0} tracklets, but parser found {1}!'.format(num_tracklets, tracklet_idx))
    return tracklets
