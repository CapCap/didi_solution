...
def main():
    ...
    parser.add_argument('-t', '--ts_src', type=str, nargs='?', default='obs_rec',
    ...
    filter_topics = CAMERA_TOPICS + CAP_FRONT_RTK_TOPICS + CAP_REAR_RTK_TOPICS \
                    + CAP_FRONT_GPS_TOPICS + CAP_REAR_GPS_TOPICS + VELODYNE_TOPICS
    ...
    for bs in bagsets:
        ...
        if len(camera_dict['timestamp']):
            ...
            interpolate_and_save(cap_rear_gps_df, gps_cols, 'capture_vehicle_rear_gps_interp.csv')
            interpolate_and_save(cap_front_gps_df, gps_cols, 'capture_vehicle_front_gps_interp.csv')
            cap_rear_rtk_interp = interpolate_and_save(cap_rear_rtk_df, rtk_cols, 'capture_vehicle_rear_rtk_interp.csv')
            cap_front_rtk_interp = interpolate_and_save(cap_front_rtk_df, rtk_cols, 'capture_vehicle_front_rtk_interp.csv')
            ...
            for obs_name in obstacle_rtk_dicts.keys():
                ...
                obs_tracklet = Tracklet(object_type=md['object_type'], l=md['l'], w=md['w'], h=md['h'], first_frame=0)
